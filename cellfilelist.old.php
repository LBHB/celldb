...
echo( "<p><b>$siteinfo Preprocessed data query</b>\n" );
echo("&nbsp;<a href=\"celllist.php\">Cell List</a><p>\n");
...
$rawsql="SELECT gDataRaw.*,gSingleCell.cellid as scellid,".
     ...
     " AND gDataRaw.bad=0" .
     $swhere .
     " ORDER BY $sorder gCellMaster.animal,gSingleCell.cellid,gDataRaw.id";
//echo("sql: $rawsql<br>\n");
...
echo(" Resp fmt: <select name=\"respfmtcode\" size=1>");
if ($respfmtcode == -1) {
  echo(" <option value=\"-1\" selected>All</option>");
} else {
  echo(" <option value=\"-1\">All</option>");
}
$fmtdata=mysql_query("SELECT DISTINCT respfmtcode,respfilefmt".
                     " FROM sCellFile" .
                     " ORDER BY respfmtcode");
while ( $row = mysql_fetch_array($fmtdata) ) {
   if ($respfmtcode == $row["respfmtcode"]) {
       $sel=" selected";
   } else {
       $sel="";
   }
   echo(" <option value=" . $row["respfmtcode"] . "$sel>" . $row["respfilefmt"] . "</option>");
}
echo(" </select><br>\n");
...
echo("    <td><b>CFID</b></td>\n");
echo("    <td><b>" . $sorturl . "gSingleCell.cellid\">Cell</a></b><br></td>\n");
...
while ( $rawrow = mysql_fetch_array($rawfiledata) ) {
  ...
  if (""!=$respfmtcode && $respfmtcode>-1) {
    $swhere=" AND sCellFile.respfmtcode=$respfmtcode";
  } else{
    $swhere="";
  }
  ...
  while ( $row = mysql_fetch_array($cellfiledata) ) {
    ...
    echo("   <td>" . $row["respfilefmt"] . "</td>\n");
    echo("   <td>" . $row["repcount"] . "</td>\n");
    echo("   <td>" . $rawrow["trials"] . "</td>\n");
  ...
  if ( 0 == $filecount && $showunproc) {
    ...
    echo($rawrow["id"] . "</a>&nbsp;</td>\n");
    echo("   <td>--</td>\n");
    echo("   <td>" . $rawrow["scellid"] . "</td>\n");
    echo("   <td>$area</td>\n");
    echo("   <td>$runclass</td>\n");
    //echo("   <td>" . $rawrow["stimspeedid"] . " Hz</td>\n");
    echo("   <td>--</td>\n");
    echo("   <td>" . $rawrow["reps"] . "</td>\n");
    echo("   <td>" . $rawrow["trials"] . "</td>\n");
    //echo("   <td>* $stimfile *</td>\n");
    if (""==$rawrow["matlabfile"]) {
      echo("   <td>* " . $rawrow["respfile"] . " *</td>\n");
    } else {
      echo("   <td>" . basename($rawrow["matlabfile"]) . "</td>\n");
    }
